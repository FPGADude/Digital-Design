...
module gpr_file(
    ...
    gpr R0(.clk(clk), .reset(reset), .s(s_r0), .e(e_r0), .d_in(d_in), .d_out(o_r0));
    gpr R1(.clk(clk), .reset(reset), .s(s_r1), .e(e_r1), .d_in(d_in), .d_out(o_r1));
    gpr R2(.clk(clk), .reset(reset), .s(s_r2), .e(e_r2), .d_in(d_in), .d_out(o_r2));
    gpr R3(.clk(clk), .reset(reset), .s(s_r3), .e(e_r3), .d_in(d_in), .d_out(o_r3));

    assign d_out = e_r0 ? o_r0 :
                   e_r1 ? o_r1 :
                   e_r2 ? o_r2 :
                   e_r3 ? o_r3 : 8'h00;
