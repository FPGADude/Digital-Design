...
// Included Modules - RAM256, ALU, CONTROL UNIT, TMP, BUS1, DATA BUS, GPR FILE, IR, IAR, ACC, FLAGS, IO ---------------------------------
		ram RAM256(
		  	.clk(in_clk),
		  	.reset(reset),
			.a(mar_in),								// 8-bit memory address from DATA BUS
			.sa(set_mar),						// set memory address in memory address register
			.s(set_ram),								// set data in memory at address
			.e(e_ram),								// enable data out from memory at address
			.d_in(ram_in),							// 8-bit data to RAM from DATA BUS
